...
# ---------- header ----------
st.title("📍 SERP Grid")
st.caption("Mobile Google rank check from local points within a service area.")
        ...
        rows.append(
            {
                ...
                "Keyword": r["keyword"],
                "Lat": round(r["lat"], 5) if pd.notna(r["lat"]) else "—",
                "Lng": round(r["lng"], 5) if pd.notna(r["lng"]) else "—",
...
for i in range(0, len(run_df), COLS):
    ...
    for j, (_, r) in enumerate(chunk.iterrows()):
        with cols[j]:
            ...
            cap = f"**#{int(r['point_index']) + 1}**"
            if r.get("label"):
                cap += f" · {r['label']}"
            if pd.notna(r.get("lat")):
                cap += f" · {r['lat']:.4f}, {r['lng']:.4f}"
            cap += f" · *{r['keyword']}*"
            st.caption(cap)
